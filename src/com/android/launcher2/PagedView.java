...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected final static int TOUCH_STATE_PRE_SCROLLING = 4;
    ...
    private boolean mInterestingTouch = false;

    protected void notifyUninterestingTouch() {
        mInterestingTouch = false;
    }

    private void markTouchInteresting() {
        mInterestingTouch = true;
    }
    ...
    public void requestDisallowInterceptTouchEvent(boolean disallowIntercept) {
        if (disallowIntercept) {
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING ||
                    mTouchState == TOUCH_STATE_PRE_SCROLLING) {
                /*Unfortunately, we can't just call onScrollingActionUp
                 * since we don't have a motion event.  However, we can still
                 * snapToDestination.  This will usually just release
                 * the Launcher back to the rest position
                 */
                snapToDestination();
            }
            onActionUpOrCancel();
    ...
    private void setScrollingStartPos(final float x) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
         ...
         * scrolling there.
         *
         * We also do actual handling for pre-scrolling here.
         */
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                 ...
                 * If we are pre-scrolling, do the scrolling here
                 */
                if (mActivePointerId != INVALID_POINTER) {
                    determineScrollingStart(ev);
                    if (mTouchState == TOUCH_STATE_PRE_SCROLLING ||
                            mTouchState == TOUCH_STATE_SCROLLING) {
                        onScrollingActionMove(ev);
                    }
                    break;
                }
         ...
         * drag mode.  Don't intercept if we are in pre-scrolling, since we want
         * to give children a chance to handle
         */
        return (mTouchState != TOUCH_STATE_REST) && (mTouchState != TOUCH_STATE_PRE_SCROLLING);
    }

    private void determinePreScrollingStart(final float x, final int xDiff,
            final int yDiff, final boolean xPaged) {
        if (mTouchState != TOUCH_STATE_PRE_SCROLLING &&
                mTouchState != TOUCH_STATE_SCROLLING &&
                (xDiff > 0 || yDiff > 0 || xPaged) &&
                !mInterestingTouch) {
            mTouchState = TOUCH_STATE_PRE_SCROLLING;
            setScrollingStartPos(x);
            pageBeginMoving();
            /* HACK
             *  Invalidate here because otherwise calling setScrollingStartPos 
             *  above means there will be no delta for this frame
             *  and we won't invalidate the UI later.  This costs a frame of delay
             */
            invalidate();
        }
    }
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        if (xMoved || xPaged || yMoved) {
            if (mUsePagingTouchSlop ? xPaged : xMoved) {
                boolean werePreScrolling = mTouchState == TOUCH_STATE_PRE_SCROLLING;
                mTouchState = TOUCH_STATE_SCROLLING;
                //
                //If we were pre-scrolling, we've already set the start pos!
                if (!werePreScrolling) {
                    setScrollingStartPos(x);
                    pageBeginMoving();
                }
            ...
            determinePreScrollingStart(x, xDiff, yDiff, xPaged);
        }
    }
    ...
    private void onScrollingActionMove(MotionEvent ev) {
    ...
    private void onScrollingActionUp(MotionEvent ev) {
